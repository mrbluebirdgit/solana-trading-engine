...
The repository now contains structural `TokenObservation` and `TrafficSnapshot` validation for these distinctions. The Pump/PumpSwap collectors, canonical stage resolver, launch-cohort reconstruction and canonical migration-LP verifier are still unimplemented; this ledger does not claim those facts are already being collected.
